...
from tfutils.utils import restore_latest_checkpoint
from training_utils import get_logger, StatisticsKeyComparator, get_statistics, save_dict
...
class Trainer(Module):
        ...
        if self.mode in (Module.TRAIN, Module.EVALUATE):
            ...
            if self.mode == Module.TRAIN:
                ...
                if not hasattr(self, 'optimizer'):
                    ...
                    vars = self.collect_dict(Module.TRAINABLE_WEIGHTS, dict_values=True)
                    if len(vars) != len(tf.trainable_variables()):
                        warnings.warn('{}/{} variables trained.'.format(len(vars), len(tf.trainable_variables())))
    ...
    def fit(self, data,
        ...
        data = get_dataloader(data, batch_size, shuffle=shuffle)
        if validation_data:
            validation_data = get_dataloader(validation_data, batch_size, shuffle=False)
    ...
    def evaluate(self, data, batch_size=32, callbacks=None, verbose=1):
        ...
        data = get_dataloader(data, batch_size=batch_size, shuffle=False, drop_last=False)
